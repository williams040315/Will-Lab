...
for fichier in list_file:
    ...
    for angle in range(0, 360, 30):
        ...
        #cv2.imwrite('./create/imgCreate_'+str(a)+'.png',img[:576,:560])
        img_mask=ti.rotateImage(img_mask_orig, angle)
        #cv2.imwrite('./create/maskCreate_'+str(a)+'.png',img_mask[:576,:560])
        tab_masks.append(img_mask[:576, :560])
        
        img=cv2.flip(img_r, 0)
        img=ti.random_change(img)
        tab_images.append(img[:576, :560])
        #cv2.imwrite('./create/imgCreate_'+str(a)+'.tif',img[:576,:560])
        img_m=cv2.flip(img_mask, 0)
        #cv2.imwrite('./create/maskCreate_'+str(a)+'.tif',img_m[:576,:560])
